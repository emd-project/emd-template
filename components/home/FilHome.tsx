...
export function FilHome({ locale = niche.defaultLocale }: { locale?: string }) {
  const f = (k: string) => tl(locale, `homeFil.${k}`)
  ...
  return (
    <main id="main-content" className="fil-page">
      ...
      <div className="fil-status">
        <div className="wrap">
          <span className="fil-live"><span className="fil-pdot" /> {f('live')}</span>
          <div className="right"><span><b>{articles.length}</b> {f('articlesWeek')}</span><span>{niche.siteName}</span></div>
        </div>
      </div>

      {breaking.length > 0 && (
        <div className="fil-breaking">
          <span className="lab"><span className="wd" /> {f('breaking')}</span>
      ...
      <header className="fil-hero" id="fil">
        ...
        <div className="wrap">
          <div className="fil-feature">
            {feat && (
              <Link href={href(feat)} className="ffeat on">
                <Cover a={feat} />
                <div className="body">
                  <span className="flag"><span className={`tag c${catN(feat.categorie)}`}><span className="pip" />{f('frontPage')} · {catLabel(feat.categorie)}</span></span>
                  <h2>{feat.title}</h2>
                  {feat.description && <p>{feat.description}</p>}
                  <div className="meta">{fmt(feat.publishedAt)} · {feat.readingTimeMin} {f('min')}</div>
                </div>
              </Link>
            )}
          </div>
          ...
          <aside className="fil-live-col">
            <div className="fil-live-head"><span className="fil-pdot" /> {f('liveFeed')}</div>
            ...
            <div className="fil-live-foot">
              <Link href={lp('/blog')}>{f('seeFeed')} <Arrow /></Link>
            </div>
          </aside>
        </div>
      </header>
        ...
        <section className="fil-rub" id="rubriques">
          <div className="wrap">
            <div className="fil-sec-head">
              <h2>{f('rubTitle')}</h2>
              <Link href={lp('/blog')} className="more">{f('allArticles')} <Arrow /></Link>
            </div>
      ...
      <section className="fil-mid" id="plus-lu">
        <div className="wrap">
          <div>
            <div className="trend-head"><TrendIc /> {f('mostRead')}</div>
            <div className="trend">
              {top.map((a, i) => (
                <Link href={href(a)} className="trd" key={a.slug}>
                  ...
                  <span className="trv">{a.readingTimeMin} {f('min')}</span>
                </Link>
              ))}
            </div>
          </div>
          ...
          <div className="fil-quote">
            ...
            <span className="eyebrow">{f('edito')}</span>
