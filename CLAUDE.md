...
- **Carte d'intentions — UNE intention = UN asset (anti-cannibalisation, blog ↔ assets ET assets ↔ assets) :**

  | Intention recherchée | Asset propriétaire | Exemples de requêtes |
  |---|---|---|
  | « meilleur / top / classement / les meilleurs X » | **Classement** `/classement/X` | quels sont les meilleurs X · top 10 X · classement X |
  | « X vs Y / comparer / X ou Y » (items précis) | **Comparateur** `/comparer/X` | X vs Y · comparer X et Y · différence X Y |
  | « quel X choisir / quel X pour [profil] / lequel » | **Choisir / Quiz** `/choisir/X`, `/quiz` | quel X choisir · quel X pour une famille · lequel prendre |
  | informationnel (comment / pourquoi / qu'est-ce que / prix / définition / avantages-inconvénients / use-case) | **Blog** | comment choisir un X · pourquoi X · prix d'un X · X est-il fiable |

  Le **blog ne cible JAMAIS** un terme tête appartenant à un des 3 assets commerciaux ; il **maille vers** l'asset. Et les 3 assets **ne se chevauchent pas** entre eux (liste rankée ≠ comparaison 1:1 ≠ reco personnalisée).
- **Réservation à l'init (mécanique, pas déclarative) :** chaque terme tête est **attribué à son asset** et **retiré du `calendrier-edito`** (blog) ; il est listé dans `content/mots-cles.md` → « requêtes à ÉVITER » côté blog. Cf. `skills/seo-geo-redaction` + skills d'init. → zéro cannibalisation.
- **Volume minimum : une page classement fait ≥ 1000 mots** (intro + TL;DR + `criteria` + `methodology` + `sources` + `faq` + par item `verdict`/`pros`/`cons`/`bestFor`). En dessous = page thin, non citable → enrichir le JSON (pas le template).
...
Le blog reste **informationnel** : il ne double JAMAIS un asset commercial (classement / comparateur / choisir) — cf. « Classements », carte d'intentions.
...
Exécuter **`docs/AUTO-DESIGN.md`** : sélection auto de la variante (home/catégorie) + permutations
(`shape`/`border`/`shadow`) via `suggestVariants(domaine)`, palette via `niche.config.palette` →
`globals.css`, typo via `suggestFonts` (`lib/typography.ts`) → `layout.tsx`. Un site qui sort de l'init
avec le thème par défaut, sans variante choisie, ou avec une valeur écrite dans `volteo.css :root` = **bug d'init**.

## Images — SOBRE à l'init (plafond strict)
À l'init, après AUTO-DESIGN, générer **AU PLUS ~5 images** : le(s) visuel(s) du **hero** de la home +
la **couverture du hub** `/blog` + illustration/fond **par catégorie** (servent aussi de réemploi
in-content des articles). **INTERDIT** de générer une image par article à l'init. Le reste se génère
par la **tâche de rédaction quotidienne** (1 cover généré + 2 in-content réutilisées). `next/image`
uniquement · `alt` descriptif FR + EN. Cf. `docs/IMAGES-WORKFLOW.md`.
...
(design-system Voltéo) sont branchées dessus dans `app/styles/volteo.css`. Pool typo : `lib/typography.ts`.
...
- [ ] **Anti-cannibalisation** : aucun article blog sur un terme tête commercial (meilleur/top/vs/quel choisir) — réservé aux assets classement/comparateur/choisir
